...
import { OscilloscopeData } from '../interfaces/hardware';

export function getRigolData(ip: string, port: number): Promise<OscilloscopeData> {
    return new Promise((resolve) => {
        ...
        const processAndResolve = () => {
            ...
            try {
                ...
                const responses: Partial<OscilloscopeData> = {};
                ...
                const preamble = lines[3].split(',');
                
                if (preamble.length < 10) throw new Error(`Preamble incompleto: "${lines[3]}".`);

                responses.timeScale = parseFloat(preamble[4]); // Xincrement
                responses.voltageOffset = parseFloat(preamble[8]); // Yorigin
                ...
                const yIncrement = parseFloat(preamble[7]); // Factor de escala vertical
                const yOrigin = parseFloat(preamble[8]);    // Origen de datos vertical
                const yReference = parseFloat(preamble[9]); // Punto de referencia vertical
                ...
                const processedWaveform: number[] = [];
                ...
                resolve({ status: 'success', ...responses } as OscilloscopeData);
        ...
        client.on('data', (data) => {
            ...
            if (!binaryBlockStartDetected) {
                const bufferAsString = receivedData.toString('ascii');
                binaryStartIndex = bufferAsString.indexOf('#');

                if (binaryStartIndex !== -1) {
                    binaryBlockStartDetected = true;
                    if (receivedData.length >= binaryStartIndex + 2) {
                        try {
                            numDigits = parseInt(receivedData.toString('ascii', binaryStartIndex + 1, binaryStartIndex + 2), 10);
                            if (!isNaN(numDigits) && numDigits >= 1 && numDigits <= 9) {
