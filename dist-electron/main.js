...
function generateReportHtml(project, points, dims) {
  ...
  const notesHtml = project.notes ? `<div class="notes-section"><h3>Notas del Proyecto</h3><p>${project.notes.replace(/\n/g, "<br>")}</p></div>` : "";
  const pointsA = points.filter((p) => !p.side || p.side === "A");
  const pointsB = points.filter((p) => p.side === "B");
  const renderBoardWithOverlays = (imgSrc, pts, width, height, side = "A") => {
    if (!imgSrc) return "<p>No hay imagen disponible.</p>";
    const gap = 48;
    const offset = side === "B" && (dims == null ? void 0 : dims.widthA) ? dims.widthA + gap : 0;
  ...
  return `
        ...
        <body>
            ...
            <div class="side-section">
                <h2>Lado A (Top)</h2>
                ${renderBoardWithOverlays(imageA, pointsA, dims == null ? void 0 : dims.widthA, dims == null ? void 0 : dims.heightA, "A")}
            ...
            <div class="side-section">
                <h2>Lado B (Bottom)</h2>
                ${renderBoardWithOverlays(imageB, pointsB, dims == null ? void 0 : dims.widthB, dims == null ? void 0 : dims.heightB, "B")}
  ...
  try {
    ...
    try {
      if (project.image_data) {
        const img = electron.nativeImage.createFromBuffer(Buffer.from(project.image_data));
      ...
      if (project.image_data_b) {
        const img = electron.nativeImage.createFromBuffer(Buffer.from(project.image_data_b));
        const size = img.getSize();
        if (size.width > 0 && size.height > 0) {
